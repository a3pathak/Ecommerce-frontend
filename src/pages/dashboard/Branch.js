...
export default function EcommerceProductList() {
  ...
  const handleDeleteBranches = () => {
    // const deleteBranches = productList?.filter((product) => !selected.includes(product.name));
    // setSelected([]);
    // setProductList(deleteBranches);
  };

  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - productList?.length) : 0;

  const filteredBranches = applySortFilter(productList, getComparator(order, orderBy), filterName);

  const isNotFound = !filteredBranches?.length && Boolean(filterName);
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        ...
        <Card>
          <ProductListToolbar
            ...
            onDeleteBranches={() => handleDeleteBranches(selected)}
          />
          ...
          <Scrollbar>
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                ...
                <TableBody>
                  {filteredBranches?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return array?.filter((Branches) => Branches.name.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
